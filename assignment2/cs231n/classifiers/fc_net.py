...
class FullyConnectedNet(object):
  ...
  def loss(self, X, y=None):
    ...
    first_hidden_layer_score = np.maximum(0, np.dot(X, self.params['W1']) 
                                          + self.params['b1'])
    hidden_layer_score.append(first_hidden_layer_score)
    
    #evaluate the middel hidden layers
    for i in range(1, self.num_layers-1):
        current_hl_score = np.maximum(0, np.dot(hidden_layer_score[i-1], self.params['W'+str(i+1)])
                                      + self.params['b'+str(i+1)])
        hidden_layer_score.append(current_hl_score)
        
    # evaluate the output layer
    scores = np.dot(hidden_layer_score[-1], self.params['W'+str(self.num_layers)]) 
